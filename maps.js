...
function StationFormulaire(mymap, station, eltHtmlStation, eltHtmlReservation, eltHtmlFormulaire, eltHtmlPresentation) {
  ...
  //En fonction de l'état de la station affiche le commentaire ad oc'
  ...
  // methode qui affiche le formulaire si besoin et determine ce qu'il faut afficher dans le formulaire
...
ajaxGet(url, function (reponse) {
  ...
  // on crée dans chaque cas une  nouvelle instance de la class StationFormulaire
  var stations = JSON.parse(reponse);
  stations.forEach(function (station) {
    if (Date.now() - sessionStorage.dateReservation <= 1200000) {
      valid.StartFunction();
      valid.ResetFunction();
      if (station.name === sessionStorage.station_name) {
